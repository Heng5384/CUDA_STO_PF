...
XB0="${XB0:-0.03}"
RESULT_ROOT="${RESULT_ROOT:-$ROOT_DIR/Results/ch_T380_cuda_96x96x96_phi_eta_step_delta}"
SCRIPT_OUT="${SCRIPT_OUT:-$ROOT_DIR/reports/data/step38e_phi_eta_step_delta}"
...
drop_prefixes = {
    ...
    "ic_vf_target_phi=",
    "ic_vf_init_phi=",
    "phi_eta_step_delta_diag_enabled=",
    "phi_eta_step_delta_diag_every=",
    "phi_eta_step_delta_diag_max_steps=",
    "phi_eta_step_delta_diag_prefix=",
}
...
append = [
    ...
    "ic_23d_xB_out=${XB0}",
    "diag_vtk_enabled=0",
    "phi_eta_step_delta_diag_enabled=1",
    "phi_eta_step_delta_diag_every=1",
    "phi_eta_step_delta_diag_max_steps=${nsteps}",
    "phi_eta_step_delta_diag_prefix=phi_eta_step_delta",
]
out.write_text("\n".join(kept + append) + "\n", encoding="utf-8")
PY
...
JSON
  ...
  echo "[step38e] running $case_name f_eta=$f_eta gp_L_eta=$gp_L_eta"
...
run_case "gp_eta_kinetic_scan_f1em5" "1e-5" "500" "100"
run_case "gp_eta_kinetic_scan_f1em4" "1e-4" "500" "100"
run_case "gp_eta_kinetic_scan_f1em3" "1e-3" "50" "10"

python3 "$ROOT_DIR/analysis/postprocess_phi_eta_step_delta.py" \
  --cases "${CASE_DIRS[@]}" \
  --out-dir "$SCRIPT_OUT"

echo "[done] step38e phi-vs-eta step-delta diagnostics complete"
