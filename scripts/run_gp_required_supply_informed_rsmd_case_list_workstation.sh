...
OUT_ROOT="${OUT_ROOT:-tmp_codex_ops/gp_required_supply_informed_rsmd_case_list}"
REPORT_ROOT="${REPORT_ROOT:-reports/gp_required_supply_informed_rsmd_case_list}"
...
if [ -z "$CASE_LIST_FILE" ]; then
  echo "[abort] CASE_LIST_FILE is required" >&2
  exit 64
fi
...
if pgrep -af main_cuda | grep -v "run_gp_required_supply_informed_rsmd" >/tmp/gp_required_supply_rsmd_main_cuda_running.txt; then
  echo "[abort] main_cuda is already running; not starting GP required-supply-informed RSMD case-list sweep." >&2
...
python3 - "$manifest" "$CASE_LIST_FILE" <<'PY' >"$OUT_ROOT/selected_cases.tsv"
import csv
import sys
from pathlib import Path

manifest, case_list_file = sys.argv[1], sys.argv[2]
requested = []
seen = set()
for raw in Path(case_list_file).read_text().splitlines():
    case = raw.strip()
...
missing = [case for case in requested if case not in rows]
if missing:
    raise SystemExit("requested case(s) missing from manifest: " + ", ".join(missing))
...
echo "gp_required_supply_informed_rsmd_case_list_workstation_complete"
