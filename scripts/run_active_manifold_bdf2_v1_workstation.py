#!/usr/bin/env python3
"""Run frozen T400 active-manifold IMEX-BDF2 replays on workstation only."""
...
def run_case(host: str, name: str, steps: int, label: str) -> dict[str, object]:
    case = CASES[name]
    ensure_idle(host)
    source = f"{REMOTE_REPO}/runs/T400_longtime_v1/coarse4_runs/{case['source']}"
    checkpoint = f"{source}/run/Results/{case['result_parent']}/{case['source']}"
    tag = f"{name}_active_manifold_{label}_{steps}"
...
def main() -> int:
    ...
    parser.add_argument("--label", default="event_window")
    args = parser.parse_args()
    selected = list(CASES) if args.case == "both" else [args.case]
    results = [
        run_case(args.host, case, args.steps, args.label) for case in selected
    ]
