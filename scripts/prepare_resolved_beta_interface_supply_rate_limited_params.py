#!/usr/bin/env python3
"""Prepare rate-limited GP-to-interface required-supply relay diagnostics.

The interface shell geometry is retained, but the source cap is reduced from
5% to 0.1% of eligible active inventory per code step.  This retains the same
PF equation and bounded-reservoir bookkeeping while avoiding the documented
overdrive case in which a narrow shell develops unphysical xB saturation.
"""
...
OUT = ROOT / "params/resolved_beta_interface_supply_rate_limited"
...
def main() -> None:
    ...
    for temp, base_name in BASES.items():
        ...
        label = f"{temp}_upper_guard_dt0p01_interface_shell2nm_f0p001_hsrc0p49"
        values.update({
            ...
            "diagnostic_rsmd_f_max_per_step": "1.0000000000000000e-03",
        ...
        write_params(param_path, order, values, f"""# Rate-limited GP-to-interface required-supply relay diagnostic
# Base: {base_name}
# Case: {label}
# This uses a bounded reservoir, matrix-side interface shell, and a reduced
# per-step source cap to test growth without concentration-overdrive artifacts.
# It remains a scenario diagnostic, not a calibrated GP release law.
""")
        rows.append({
            "case": label,
            ...
            "diagnostic_rsmd_f_max_per_step": 0.001,
            "diagnostic_rsmd_h_src_max": 0.49,
            ...
            "physical_claim": "rate_limited_required_supply_relay_diagnostic_not_calibrated_GP_thermodynamics",
        })
