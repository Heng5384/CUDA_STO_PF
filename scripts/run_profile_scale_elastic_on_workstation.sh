...
OUT_ROOT="${OUT_ROOT:-tmp_codex_ops/profile_scale_calibration_elastic_on}"

cd "$ROOT"
mkdir -p "$OUT_ROOT"

if pgrep -af main_cuda | grep -v "run_profile_scale_elastic_on_workstation" >/tmp/profile_scale_elastic_on_running.txt; then
  echo "[abort] main_cuda is already running; not starting elastic-on diagnostics." >&2
  cat /tmp/profile_scale_elastic_on_running.txt >&2
  exit 75
fi
...
run_case() {
  ...
  mkdir -p "$dir"
  echo "[run] $name scale=$scale dt=$dt elastic=1"
  set +e
  timeout 5400 ./main_cuda \
...
run_case T400_elastic_scale1_dt0p02 1.0 0.02
run_case T400_elastic_scale2_dt0p02 2.0 0.02
run_case T400_elastic_scale3_dt0p02 3.0 0.02
run_case T400_elastic_scale4_dt0p02 4.0 0.02
run_case T400_elastic_scale5_dt0p02 5.0 0.02
run_case T400_elastic_scale6p666_dt0p02 6.6666666667 0.02

echo "profile_scale_elastic_on_workstation_jobs_complete"
