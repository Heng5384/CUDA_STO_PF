#!/usr/bin/env python3
"""Read-only completion audit for the independent 400^3 elastic PSD pilot."""
...
PASS = "PASS_400CUBE_FIXED_XB03_ELASTIC_PSD_SIGN_PILOT_V1"
BLOCKED = "BLOCKED_400CUBE_FIXED_XB03_ELASTIC_PSD_SIGN_PILOT_V1"
FINAL_STEP = 152585
...
def main() -> None:
    ...
    try:
        ...
        if initial_count != 64 or int(observations[0]["particle_count"]) != initial_count:
            raise ValueError("initial 64-particle PSD registration mismatch")
        ...
        for index, step in enumerate(endpoints):
            ...
            if (segment / "status.txt").read_text().strip() != "PASS_400CUBE_FIXED_XB03_ELASTIC_PSD_SIGN_SEGMENT_V1":
        ...
        audit = {"schema": "PF_400CUBE_FIXED_XB03_ELASTIC_PSD_SIGN_PILOT_AUDIT_V1", "status": PASS if all(gates.values()) else BLOCKED, "gates": gates, "scientific_status": scientific, "checkpoint_hashes": checkpoint_hashes, "metrics": {"initial_particle_count": initial_count, "final_particle_count": int(final["particle_count"]), "initial_mean_radius_nm": float(hourly[0]["mean_radius_nm"]), "final_mean_radius_nm": float(final["mean_radius_nm"]), "final_far_field_matrix_xAg": float(final["far_field_matrix_xAg"]), "max_system_mass_drift": mass_drift, "max_zero_mode_lambda": max_lambda, "wall_seconds": wall_seconds, "wall_seconds_per_step": wall_seconds / FINAL_STEP if wall_seconds else None, **gpu_metrics(args.gpu_samples)}, "qualification_scope": "user-authorized direct 400-cube pilot; one-step/restart and short physical smoke were intentionally skipped; not box-converged, ensemble-qualified, or an absolute experimental reconstruction"}
        (args.out / "audit.json").write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        (args.out / "status.txt").write_text(audit["status"] + "\n", encoding="utf-8")
        (args.out / "final_terminal_output.txt").write_text("\n".join(f"{key}={value}" for key, value in {"final_status": audit["status"], "identity_status": "PASS" if gates["merge_aware_identity"] else "FAIL_CLOSED", "max_system_mass_drift": mass_drift, "max_zero_mode_lambda": max_lambda, "final_particle_count": final["particle_count"], "final_mean_radius_nm": final["mean_radius_nm"], "final_far_field_xAg": final["far_field_matrix_xAg"]}.items()) + "\n", encoding="utf-8")
        print(audit["status"])
