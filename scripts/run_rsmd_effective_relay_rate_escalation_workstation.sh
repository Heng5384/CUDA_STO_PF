...
MANIFEST="${MANIFEST:-params/rsmd_effective_relay_rate_escalation/rsmd_effective_relay_rate_escalation_manifest.csv}"
...
python3 scripts/prepare_rsmd_effective_relay_rate_escalation.py
...
PY
  ...
  timeout "$TIMEOUT_S" ./main_cuda --pf-param-file "$param_file" \
    ...
    --csv-out-every 50 --init-case-tag "$run_id" >"$case_dir/stdout.log" 2>"$case_dir/stderr.log"
  rc=$?
  set -e
...
echo rsmd_effective_relay_rate_escalation_complete
