...
MANIFEST="${MANIFEST:-params/rsmd_y_solver_attribution/rsmd_y_solver_attribution_manifest.csv}"
...
python3 scripts/prepare_rsmd_y_solver_attribution.py
...
PY
  ...
  timeout "$TIMEOUT_S" ./main_cuda --pf-param-file "$param_file" \
    ...
    --csv-out-every 25 --init-case-tag "$run_id" >"$case_dir/stdout.log" 2>"$case_dir/stderr.log"
  rc=$?
  set -e
...
echo rsmd_y_solver_attribution_complete
