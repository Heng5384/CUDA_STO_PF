...
OUT_ROOT="${OUT_ROOT:-tmp_codex_ops/beta_seed_xBcrit_titration_with_two_radius_extension}"
REPORT_ROOT="${REPORT_ROOT:-reports/beta_seed_xBcrit_titration_with_two_radius_extension}"
NPOST="${NPOST:-1000}"
...
if pgrep -af main_cuda | grep -v "run_beta_seed_xBcrit_two_radius_extension_workstation" >/tmp/beta_seed_xBcrit_two_radius_main_cuda_running.txt; then
  echo "[abort] main_cuda is already running; not starting beta seed xBcrit two-radius titration." >&2
  cat /tmp/beta_seed_xBcrit_two_radius_main_cuda_running.txt >&2
  exit 75
fi
...
if [ "$build_rc" -ne 0 ]; then
  ...
  python3 scripts/generate_beta_seed_xBcrit_two_radius_extension_audit.py >"$OUT_ROOT/report_generator.stdout.log" 2>"$OUT_ROOT/report_generator.stderr.log" || true
  exit "$build_rc"
fi
...
python3 scripts/analyze_beta_seed_xBcrit_titration.py \
  ...
  2>"$OUT_ROOT/analyzer.stderr.log" || true

python3 scripts/generate_beta_seed_xBcrit_two_radius_extension_audit.py \
  >"$OUT_ROOT/two_radius_report_generator.stdout.log" \
  2>"$OUT_ROOT/two_radius_report_generator.stderr.log"

echo "beta_seed_xBcrit_two_radius_extension_workstation_jobs_complete"
