...
int main(int argc, char** argv) {
    try {
        ...
        std::vector<double> x_values = {
            ...
            0.01, 0.02, 0.05, 0.08, 0.09, 0.1, 0.2};
        ...
        std::cout
            << "contract_hash,temperature_K,xB,radius_m,G_alpha_J_mol,mu_A_J_mol,"
               ...
               "Vm_alpha_m3_mol,Vm_beta_m3_mol,convex_extrapolation_enabled\n";
        for (double xB : x_values) {
            for (double radius_nm : radii_nm) {
                ...
                std::cout << PF_KWN_VALIDATION_CONTRACT_HASH << ',' << temperature_K << ','
                          ...
                          << mu_PbTe_raw(temperature_K, xB) << ','
                          << mu_Ag2Te_raw(temperature_K, xB) << ','
                          << (mu_Ag2Te_raw(temperature_K, xB) -
                              mu_PbTe_raw(temperature_K, xB)) << ','
                          << (dmu_Ag2Te_raw_dx(temperature_K, xB) -
                              dmu_PbTe_raw_dx(temperature_K, xB)) << ','
                          ...
                          << thermo_convex_extrapolation_enabled_runtime() << '\n';
            }
        }
        return 0;
