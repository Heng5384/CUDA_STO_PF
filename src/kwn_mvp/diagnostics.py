...
def records_from_history(solver: KWNSolver, history: Iterable[StepDiagnostics]) -> List[Dict[str, float]]:
    ...
    for item in history:
        row = {
            ...
            "beta_boundary_radius_m": item.beta_boundary_radius_m,
            "beta_boundary_growth_velocity_m_s": item.beta_boundary_growth_velocity_m_s,
            ...
            "active_population_courant": item.active_population_courant,
            "active_m0_courant": item.active_m0_courant,
            "active_m3_courant": item.active_m3_courant,
            "lower_tail_active_courant": item.lower_tail_active_courant,
            "boundary_candidate_courant": item.boundary_candidate_courant,
            "boundary_active_courant": item.boundary_active_courant,
            "boundary_face_is_active": item.boundary_face_is_active,
            "active_m0_support_i_lo": float(item.active_m0_support_i_lo),
            "active_m0_support_i_hi": float(item.active_m0_support_i_hi),
            "active_m3_support_i_lo": float(item.active_m3_support_i_lo),
            "active_m3_support_i_hi": float(item.active_m3_support_i_hi),
            "lower_tail_i_lo": float(item.lower_tail_i_lo),
            "lower_tail_i_hi": float(item.lower_tail_i_hi),
            "lower_tail_m0_fraction": item.lower_tail_m0_fraction,
        }
        rows.append(row)
    return rows
