...
# Create a new, non-overwriting constrained multi-particle target profile.
# This is a preparatory V3 fixture workflow only; it never starts 6--48 h
# production and never changes the selected physical contract.
...
# The two materializers own their output roots and reject pre-existing paths;
# only create the runner-owned provenance/minimizer directories here.
...
SOURCE_FILES=(
  ...
  jobs/run_pf_elastic_multi_particle_common_matrix_profile_v3.sh
)
...
PF_PARAM_FILE="$(site_generate_pf_param_file 'elastic_multi_particle_common_matrix_v3_T380')"
...
V0="$(python3 - "${RUN_ROOT}/seed/seed_manifest.json" <<'PY'
import json,sys
print(f"{json.load(open(sys.argv[1]))['inventory']['beta_volume_fraction']:.17e}")
PY
)"
...
(
  ...
  CUDA_STO_RESULTS_ROOT="${RUN_ROOT}/minimize/results" CUDA_STO_SUPPRESS_VTK_OUTPUT=1 \
    "${SOURCE_ROOT}/main_cuda" \
      ...
      --minimize-max-iter "${MAX_ITER}" --minimize-dt "${MINIMIZE_DT}" --V0 "${V0}" \
      --elastic 1 --minimize-rms-dphi-threshold "${RMS_DPHI_TOL}" \
      --minimize-rms-dY-threshold "${RMS_DY_TOL}" --minimize-rms-res-threshold "${RMS_RES_TOL}" \
      ...
      --init-meta "${RUN_ROOT}/seed/init_meta.json" --init-case-tag elastic_multi_particle_common_matrix_v3 \
      >"${RUN_ROOT}/minimize/run.stdout" 2>"${RUN_ROOT}/minimize/run.stderr"
)
[[ ! -s "${RUN_ROOT}/minimize/run.stderr" ]] || { echo "[fatal] constrained minimizer stderr" >&2; exit 2; }
grep -q '^MINIMIZE_TARGET_PROFILE_CONVERGENCE_FINAL_AUDIT status=PASS ' "${RUN_ROOT}/minimize/run.stdout"
...
[[ -n "${PHI_RAW}" && -n "${XB_RAW}" && -n "${Y_RAW}" && -n "${DY_RAW}" ]] || { echo "[fatal] minimizer raw field set incomplete" >&2; exit 2; }
...
[[ ! -s "${RUN_ROOT}/profile.stderr" ]] || { echo "[fatal] V3 profile materializer stderr" >&2; exit 2; }

cp "${RUN_ROOT}/profile/fixture_manifest.json" "${RUN_ROOT}/fixture_manifest.json"
{
  echo 'common_matrix_profile_status=PASS_PF_ELASTIC_MULTI_PARTICLE_COMMON_MATRIX_PROFILE_V3'
